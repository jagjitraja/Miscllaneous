...
public interface GuestInfoDAO {
    ...
    @Query("SELECT * FROM GuestInfoContract.TABLE_NAME ORDER BY "+ GuestInfoContract.COLUMN_GUEST_NAME+" ASC")
    public Cursor getAllGuestsInfoCursor();

    @Query("SELECT * FROM "+ GuestInfoContract.TABLE_NAME +" WHERE "+GuestInfoContract.COLUMN_GUEST_ID+" = :id")
    public Cursor getGuestInfo(long id);
